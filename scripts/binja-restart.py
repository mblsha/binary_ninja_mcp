...
class BinaryNinjaAdvancedController(cli.Application):
    ...
    timeout = cli.SwitchAttr(
        ["-t", "--timeout"], int, default=60, help="Overall timeout in seconds"
    )
    ...
    window_wait = cli.SwitchAttr(
        ["-w", "--window-wait"], int, default=30, help="Timeout for window detection"
    )
    ...
    force = cli.Flag(["-f", "--force"], help="Force kill without graceful quit")

    verbose = cli.Flag(["-v", "--verbose"], help="Enable verbose output")

    monitor_interval = cli.SwitchAttr(
        ["--monitor-interval"], float, default=0.5, help="Interval between monitoring checks"
    )
    ...
    def kill_existing(self) -> bool:
        ...
        if not self.force:
            ...
            try:
                subprocess.run(
                    ["osascript", "-e", 'tell application "Binary Ninja" to quit'],
                    capture_output=True,
                    timeout=5,
                )
        ...
        subprocess.run(
            ["pkill", "-f", "Binary Ninja\\.app/Contents/MacOS/binaryninja"], capture_output=True
        )
        time.sleep(1)
    ...
    def _is_running(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(["pgrep", "-x", "binaryninja"], capture_output=True, text=True)
            ...
            for pid in pids:
                if pid:
                    ...
                    if ps_result.returncode == 0:
                        ...
                        if "/Binary Ninja.app/Contents/MacOS/binaryninja" in path or path.endswith(
                            "/Contents/MacOS/binaryninja"
                        ):
                            return True
    ...
    def main(self, file_path: str = None):
        ...
        try:
            ...
            if self.monitor_startup(file_name):
                ...
                try:
                    subprocess.run(
                        ["osascript", "-e", 'tell application "Binary Ninja" to activate'],
                        capture_output=True,
                    )
                except Exception:
                    pass
