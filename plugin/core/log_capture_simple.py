...
class SimpleLogCapture:
    ...
    def start(self):
        ...
        if not self.is_registered:
            try:
                ...
                bn.log_debug = self._make_interceptor('DebugLog', self.original_functions['log_debug'])
                bn.log_warn = self._make_interceptor('WarningLog', self.original_functions['log_warn'])
                bn.log_error = self._make_interceptor('ErrorLog', self.original_functions['log_error'])
                bn.log_alert = self._make_interceptor('AlertLog', self.original_functions['log_alert'])
    ...
    def _make_interceptor(self, level: str, original_func):
        ...
        def interceptor(message: str, logger: str = ""):
            # Add to our buffer
            if not "[MCP]" in message:  # Skip our own logs
