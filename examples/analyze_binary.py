...
def main():
    ...
    # Categorize imports
    crypto_imports = [imp for imp in import_list if any(x in imp.lower() for x in ["crypt", "aes", "rsa", "hash", "md5", "sha"])]
    network_imports = [imp for imp in import_list if any(x in imp.lower() for x in ["socket", "recv", "send", "connect", "bind"])]
    file_imports = [imp for imp in import_list if any(x in imp.lower() for x in ["open", "read", "write", "file", "create"])]
